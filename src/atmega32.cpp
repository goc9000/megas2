...
void Atmega32::_dumpRegisters()
{
    ...
    for (int i=0; i<MEGA32_IO_BASE; i++) {
        printf("%c%02d", i ? ' ' : 'r', i);
    }
    printf("\n");
    for (int i=0; i<MEGA32_IO_BASE; i++) {
        printf(" %02x", this->ram[i]);
    }
    printf("\n");
    
    uint8_t sreg = this->_readReg(REG_SREG);
    for (int i=0; i<8; i++)
...
void Atmega32::_dumpSram()
{
    ...
    for (int i = 0; i < blocks; i++) {
        ...
        for (int j = 0; j < count; j++)
            printf(" %02x", this->ram[MEGA32_SRAM_BASE + addr + j]);
        for (int j = BYTES_PER_BLOCK; j < count; j++)
            printf("   ");
        printf(" | ");
        for (int j = 0; j < count; j++) {
            uint8_t byte = this->ram[MEGA32_SRAM_BASE + addr + j];
